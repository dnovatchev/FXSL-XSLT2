/* $Id: verbose.c,v 1.2 2007/06/01 13:33:49 dnovatchev Exp $ */
...
extern char* strip_quotes(char* str)
{
   int nstrLen = strlen(str);
	char* retstr = (char*) calloc(5*nstrLen + 1, sizeof(char));
   ...
   while(*str)
   {
	   ...
	   if( (nstrLen > 1) && (str == q || str == q + nstrLen -1) 
		  &&
		   (*str == '\'' || *str == '\"')
		   )
	   {
