/* $Id: reader.c,v 1.1 2007/05/08 13:10:38 dnovatchev Exp $ */
...
static void print_grammar_txt(void);
static void print_grammar_xml(void);
...
static void print_grammar_xml(void)
{
    register int i, j, k;
    int spacing = 0;
    register FILE *f = xml_file;

    if (!xflag)
	return;
	
	fprintf(f, "<parseTables>");
	...
	fprintf(f, "\n	<rules>");
		...
		for (i = 2; i < nrules; ++i)
		{
			...
			fprintf(f, "\n	  <r left=\"%s\" length=\"%d\">", 
				...
				{
					fprintf(f, "\n	    <right>%s</right>", 
						        strip_quotes(symbol_name[ritem[k]])
					        );
					++k;
				}
				++k;
